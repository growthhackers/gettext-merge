...
var pot = gettextParser.po.parse(fs.readFileSync(argv.pot));

// When found string "\\n" replace to "\n"
// on msgid and msgstr
var paramRemoveDoubleSlash = argv.n;
...
console.log('');
if (paramRemoveDoubleSlash) {
	console.log('Removing double slash'.green);

	removeDoubleSlash(argv.po);
	removeDoubleSlash(argv.pot);

}

function removeDoubleSlash(poFile) {
	var file = gettextParser.po.parse(fs.readFileSync(poFile));

	var keys = Object.keys(file.translations['']);
	for (var i = 1; i < keys.length; i++) {

		if (po.translations[''][keys[i]]) {
			file.translations[''][keys[i]]['msgid'] = file.translations[''][keys[i]]['msgid'].replace(/\\n/g, '\n');;
			if (file.translations[''][keys[i]]['msgstr']) {
				file.translations[''][keys[i]]['msgstr'][0] = file.translations[''][keys[i]]['msgstr'][0].replace(/\\n/g, '\n');
			}
		}
	}

	fs.writeFileSync(poFile, gettextParser.po.compile(file));
}
